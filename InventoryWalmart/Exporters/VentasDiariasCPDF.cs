...
{
    internal class VentasDiariasCPDF : ReportesVentasAutomaticos<VentasDiarias, CategoriasDIA>
    {
        public void Exportar(List<VentasDiarias> tablaGeneral, List<CategoriasDIA> tablaCategorias, string ruta)
        {
            try
            {
                ...
                Paragraph titulo = new Paragraph("REPORTE DE VENTAS DIARIAS", fuenteTitulo);
                ...
                tabla1.SetWidths(new float[] { 20f, 20f, 20f, 20f, 20f });

                string[] encabezados1 = { "Fecha", "Productos Vendidos", "Subtotal", "Descuento", "Total Vendido" };
                ...
                foreach (var venta in tablaGeneral)
                {
                    tabla1.AddCell(new Phrase(venta.fecha_reporte.ToString("dd/MM/yyyy"), fuenteCelda));
                    tabla1.AddCell(new Phrase(venta.total_productos.ToString(), fuenteCelda));
                    tabla1.AddCell(new Phrase($"${venta.sup_total:N2}", fuenteCelda));
                    tabla1.AddCell(new Phrase($"${venta.total_descuento:N2}", fuenteCelda));
                    tabla1.AddCell(new Phrase($"${venta.total_venta:N2}", fuenteCelda));
                }
                ...
                tabla2.SetWidths(new float[] { 25f, 25f, 25f, 25f });
                ...
                foreach (var cat in tablaCategorias)
                {
                    tabla2.AddCell(new Phrase(cat.fecha?.ToString("dd/MM/yyyy") ?? "N/A", fuenteCelda));
                    tabla2.AddCell(new Phrase(cat.nombreCategoria ?? "N/A", fuenteCelda));
                    tabla2.AddCell(new Phrase($"${cat.dineroVentas:N2}", fuenteCelda));
